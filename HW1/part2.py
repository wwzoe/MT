...
state_s = namedtuple("state_s" ,"k, j, i,lm_state")
...
def assign_stack(s):
  if type(s)==state_h: #when state=state_h
    return s.i
  else:
    return s.k+s.i-s.j #when state=state_s
...
def extend_state(s, f):
  if type(s)==state_h:
    ...
    for phrase in tm[f[s.k:s.j]]: #fill skipped one
      logprob=phrase.logprob
      lm_state=s.lm_state
      for word in phrase.english.split():
        (lm_state, word_logprob) = lm.score(lm_state, word)
